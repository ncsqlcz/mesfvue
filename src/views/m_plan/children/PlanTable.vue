  ...
  <el-row style="background-color: transparent">
    <el-col :span="24">
      <el-col class="toolbar">
        <el-form :inline="true" :model="currentFilters" @submit.native.prevent>
          <el-col :span="24" class="text-right">
            <!-- <el-form-item :span="4"> -->
              <!-- <el-select v-model="currentFilters.productionStatus" :placeholder="$t(`msg['选择生产状态']`)" size="medium" @change="getList">
                <el-option
            ...
            <el-form-item :span="4">
              <el-input v-model.trim="currentFilters.keyword" size="medium" :placeholder="$t(`msg['输入关键字']`)" @keyup.native.enter="getList"></el-input>
            </el-form-item>
            <el-form-item>
      ...
      <el-table :data="list" highlight-current-row
        ...
        <el-table-column :label="$t(`msg['操作']`)" width="200">
          <template slot-scope="scope">
            <el-button type="" size="small" icon="el-icon-view" @click="handleDetailView(scope.row)">{{ $t(`msg['详情']`) }}</el-button>
            <el-button v-if="detailsDialogConfig.show.operation" type="" size="small" @click="handleOperationView(scope.row)">{{ $t(`msg['操作']`) }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--列表 end-->
      ...
      <el-col :span="24" class="toolbar">
        <el-pagination layout="total, sizes, prev, pager ,next" :page-size="currentFilters.pageSize"
    ...
    <plan-operation v-if="detailsDialogConfig.show.operation"
                    :visible.sync="visible.planOperationDialog" :plan-id="planId" :config='operationBtnConfig'
                    @callback="">
    </plan-operation>
...
<script>
  ...
  import DetailsDialog from '../children/DetailsDialog.vue'
  import PlanOperation from '../children/PlanOperation.vue'
  ...
  export default {
    ...
    components: { DetailsDialog, PlanOperation },
    computed: {
      ...mapGetters({
    ...
    methods: {
      ...
      handlePageChange (val) {
        this.currentFilters.page = val
        this.getList()
      },
      // 每页数量改变
      handleSizeChange (val) {
        this.currentFilters.pageSize = val
        this.getList()
      },
      // 列表索引
      getIndex (index) {
        return utils.getHeadNumAdapter(this.currentFilters.page, this.currentFilters.pageSize, index)
      },
      // 重置
      reset () {
        this.currentFilters = this.currentFilters = Object.assign({}, this.currentFilters, this.filters)
        this.currentFilters.keyword = ''
        if (this.userId === '') {
          delete this.currentFilters.userId
        } else {
          this.currentFilters.userId = this.userId
        }
        this.getList()
      },
      // 获取列表
      getList () {
        ...
        let para = Object.assign({
          ...
          headNum: utils.getHeadNumAdapter(this.currentFilters.page, this.currentFilters.pageSize)
        }, this.currentFilters)
        this.visible.listLoading = true
        queryDraftPlanOutline(para).then((res) => {
          if (res.status === 0) {
            // this.currentFilters.page = data.currentPage
            this.currentFilters.total = res.map.line
            this.list = res.map.plans
          } else {
            this.currentFilters.total = 0
            this.list = []
          }
      ...
      // 操作
      handleOperationView (row) {
        console.log(row)
        this.planId = row.production_plan_id
        this.visible.planOperationDialog = true
        this.operationBtnConfig = {
          type: this.detailsDialogConfig.type,
          enableBtns: {
            resubmit: true,
            issued: true,
            recoverPlan: true,
