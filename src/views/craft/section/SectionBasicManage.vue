...
<template>
  <section>
    ...
    <el-col class="toolbar">
      <el-form :inline="true" :model="filters" @submit.native.prevent>
         <el-col :span="4" class="text-right">
          <el-button type="primary" size="medium" @click="AddCarftBasic">新增工艺段基础信息</el-button>
        </el-col>
    ...
    <!--列表 start-->
    <el-table :data="craftSegmentDasicsList" highlight-current-row
      ...
      <el-table-column prop="craft_segment_name" label="工艺段名称" sortable></el-table-column>
      <el-table-column prop="craft_segment_number"  label="工艺段编号" sortable></el-table-column>
      <el-table-column prop="craft_segment_basics_describe" label="工艺段备注" ></el-table-column>
...
<script>
  ...
  import {queryCraftSegmentBasics, modifyCraftSegmentBasicsStatus, saveCraftSegmentBasics
  } from '../../../api/api'
  ...
  export default {
    data () {
     return {
        ...
        addFormRules: {
           workstageName: { required: true, message: '工艺段名称不能为空', trigger: 'blur' },
            workstageNumber: { required: true, message: '工艺段编号不能为空', trigger: 'blur' }
        },
        options: [
            {value: '0', label: '启用'},
            {value: '1', label: '弃用'}
    ...
    methods: {
      ...
      getList () { 
        ...
        queryCraftSegmentBasics(para).then((res) => {
          if (res.status === 0) {
            this.filters.total = res.map.lines
             res.map.craftSegmentDasicsList.forEach(item => {
              if (item.craft_segment_basics_status === '0') {
                item.useStatus = '0'
              } else {
                item.useStatus = '1'
              }
            })
            this.craftSegmentDasicsList = res.map.craftSegmentDasicsList
          } else {
            this.filters.total = 0
            this.craftSegmentDasicsList = []
          }
        })
      },
        // 详情
      craftDetail (row) {
        if (row.craft_segment_basics_status == 0) {
            row.craft_segment_basics_status = '启用'
        }else {
             row.craft_segment_basics_status = '弃用'
        }
        row.craft_segment_basics_creation_time = this.getTime(row.craft_segment_basics_creation_time)
        this.$mesBasicInfoTable.show(
          '工序基础信息详情', {
            table: {
              props: ['工序名称', '工序编号', '工序备注', '使用状态', '创建时间', '创建人员'],
              values: [row.craft_segment_name, row.craft_segment_number, row.craft_segment_basics_describe, row.craft_segment_basics_status,
                row.craft_segment_basics_creation_time, row.craft_segment_basics_creation_staff]
            }
          })
      },
      // 修改状态
      statusChanged (row) {
        this.$confirm('确认修改状态?').then(action => {
          if (action === 'confirm') {
             let craftBasicsIds = row.craft_segment_basics_id
            if (row.useStatus === '1') {
              modifyCraftSegmentBasicsStatus({
                type: 'deprecated',
      ...
      addSubmit () {
        saveCraftSegmentBasics({
         craft_segment_name: this.addForm.workstageName,
         craft_segment_number: this.addForm.workstageNumber,
         craft_segment_basics_describe: this.addForm.workstageBasicsDescribe, 
         craft_segment_basics_status: this.addForm.workstageBasicsStatus,
         craft_segment_basics_creation_staff: this.getUser.staffNumber,
         craft_segment_basics_creation_staff_id:  this.getUser.userId
        }).then(data => {
          if (data.status === 0) {
