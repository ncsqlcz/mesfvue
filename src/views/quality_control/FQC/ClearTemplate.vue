...
<template>
  <section>
    ...
    <el-table :data="dataList" highlight-current-row v-loading="listLoading">
      ...
      <el-table-column prop="quality_fqc_template_name" label="模板名称" sortable></el-table-column>
      <el-table-column prop="quality_fqc_template_type" label="模板类型" sortable></el-table-column>
      <el-table-column prop="quality_fqc_template_edittion" label="版本" sortable></el-table-column>
...
<script>
import {queryFQCTemplate} from '../../../api/api'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      queryFQCTemplate(data).then((data) => {
        this.listLoading = false
        if (data.status === 0) {
          this.dataList = data.map.fqctemplateList
          this.total = data.map.line
