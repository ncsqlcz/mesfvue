...
<template>
  <section>
    ...
    <el-table :data="dataList" highlight-current-row v-loading="listLoading">
      ...
      <el-table-column prop="quality_fqc_report_name" label="报告名称" sortable></el-table-column>
      <el-table-column prop="quality_fqc_report_type" label="报告类型" sortable></el-table-column>
      <el-table-column prop="warehouse_product_model" label="电池型号" sortable></el-table-column>
      <el-table-column prop="warehouse_product_capacity_grade" label="电池批号" sortable></el-table-column>
...
<script>
import {queryFQCReport} from '../../../api/api'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      queryFQCReport(data).then((data) => {
        this.listLoading = false
        if (data.status === 0) {
          this.dataList = data.map.fqcReportList
