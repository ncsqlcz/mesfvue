...
<template>
  <section>
    ...
    <el-table :data="dataList" highlight-current-row v-loading="listLoading">
      ...
      <el-table-column prop="quality_pqc_template_name" label="方案名称" sortable></el-table-column>
      <el-table-column prop="quality_pqc_template_number" label="方案编号" sortable></el-table-column>
      <el-table-column prop="workstageName" label="工序" sortable></el-table-column>
      <el-table-column prop="modelName" label="检验方式" sortable></el-table-column>
      <el-table-column prop="quality_pqc_template_edittion" label="版本" sortable></el-table-column>
      <el-table-column label="创建日期" sortable>
        <template slot-scope="scope">
          {{scope.row.quality_pqc_template_date | date}}
        </template>
      </el-table-column>
...
<script>
import {queryPQCTemplate} from '../../../api/api'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      queryPQCTemplate(data).then((data) => {
        this.listLoading = false
        if (data.status === 0) {
          this.dataList = data.map.pqcTemplates
  ...
  filters: {
    date (val) {
      return utils.formatDate.format(new Date(val), 'yyyy-MM-dd')
    }
  },
  mounted () {
    this.getData()
