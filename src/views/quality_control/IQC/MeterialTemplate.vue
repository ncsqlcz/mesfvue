...
<template>
  <section>
    ...
    <el-col :span=24 :gutter="20" class="toolbar">
      <el-form :inline="true" :model="axiosData" onsubmit="return false">
        ...
        <el-col :span="20" class="text-right">
          <el-form-item>
            <el-input v-model.trim="axiosData.templateName" size="medium" placeholder="输入关键字" @keyup.native.enter="getData()"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    <el-table :data="dataList" highlight-current-row v-loading="listLoading">
      ...
      <el-table-column prop="quality_iqc_template_name" label="模板名称" sortable></el-table-column>
      <el-table-column prop="material.warehouse_material_name" label="物料名称" sortable></el-table-column>
      <el-table-column prop="quality_iqc_template_type" label="类模板名称" sortable></el-table-column>
      <el-table-column prop="quality_iqc_template_edittion" label="版本" sortable></el-table-column>
      <el-table-column label="创建时间" sortable> <template slot-scope="scope">{{ scope.row.quality_iqc_template_date | date }}</template></el-table-column>
...
<script>
import {queryIQCTemplate} from '../../../api/api'
import utils from '../../../common/js/utils'
export default {
  name: 'MeterialTemplate',
  data () {
    return {
      dataList: [],
      pageSize: 10,
  ...
  methods: {
    ...
    getData () {
      ...
      queryIQCTemplate(data).then((data) => {
        this.listLoading = false
        if (data.status === 0) {
          this.dataList = data.map.iqcTemplates
