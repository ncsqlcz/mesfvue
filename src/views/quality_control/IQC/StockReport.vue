...
<template>
  <section>
    ...
    <el-table :data="dataList" highlight-current-row v-loading="listLoading">
      ...
      <el-table-column prop="quality_iqc_report_name" label="报告名称" sortable></el-table-column>
      <el-table-column prop="material.warehouse_material_name" label="物料名称" sortable></el-table-column>
      <el-table-column prop="warehouse_material_batch" label="批次" sortable></el-table-column>
      <el-table-column prop="material.warehouse_material_standard" label="规格" sortable></el-table-column>
      <el-table-column prop="material.warehouse_material_model" label="型号" sortable></el-table-column>
      <el-table-column prop="warehouse_material_number" label="数量" sortable></el-table-column>
      <el-table-column prop="material.warehouse_material_units" label="单位" sortable></el-table-column>
      <el-table-column label="检验结果" sortable> <template slot-scope="scope">{{ scope.row.quality_iqc_comprehensive_result | result }}</template></el-table-column>
...
<script>
import {queryIQCReport} from '../../../api/api'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      queryIQCReport(data).then((data) => {
        this.listLoading = false
        if (data.status === 0) {
          this.dataList = data.map.iqcReports
