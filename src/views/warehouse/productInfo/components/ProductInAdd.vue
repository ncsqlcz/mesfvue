 ...
 * @Last Modified time: 2018-09-20 15:51:35
 */
<template>
    <!--详情 start-->
    <el-dialog :visible.sync="visibleDialog" @close="" width="80%" :close-on-click-modal="false">
      <el-row class="add-title mb-3">
        <el-col :span="6"><span class="el-dialog__title">入库登记</span></el-col>
      ...
      <el-table :data="selectedList" highlight-current-row
        ...
        <el-table-column prop="storage_quantity" label="入库数量" >
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.storage_quantity" size="small"
                              :controls="false" :min="0" style="width: 80px;"></el-input-number>
          </template>
        </el-table-column>
...
export default {
  ...
  watch: {
    visible(val) {
      ...
      if (val) {
        // 生成工单时间和单据号
        generateReciptNumber({ type: 'CPRK'}).then(res => {
  ...
  methods: {
    ...
    getList() {
      ...
      queryFinishProduct(this.filters).then(res => {
        console.log(res)
        if (res.status === 0) {
          this.filters.total = res.map.line;
          this.list = res.map.finishProductInfoDTOs;
