/*
 * @Author: liucz 
 * @Date: 2018-09-14 10:09:27 
 * @Last Modified by: liucz
 * @Last Modified time: 2018-09-21 16:28:53
 */
<template>
  <section>
    <!--搜索 start-->
    ...
    <el-table :data="productList" style="width:100%" v-loading="visible.listLoading">
      ...
      <el-table-column prop="warehouse_name" label="存储位置" sortable></el-table-column>
      ...
      <el-table-column label="操作" width="380">
          <template slot-scope="scope">
              ...
              <el-button type="primary" size="small" icon="el-icon-edit"
                          :disabled="scope.row.status === '1' || scope.row.is_on === '1'" @click="handleEdit(scope.row)">修改</el-button>
              <el-button type="danger" size="small" icon="el-icon-delete"
                          :disabled="scope.row.status === '1' || scope.row.is_on === '1'" @click="handleDelete(scope.row)">报废</el-button>
...
import {
  ...
  modifyFinishProductStorageScrap
} from "../../../api/warehouse";
...
import ProductInAdd from "./components/ProductInAdd.vue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      queryFinishProductStorage(this.filters).then(res => {
        // console.log(res)
        if (res.status === 0) {
          this.filters.total = res.map.line
          this.productList = res.map.finishStorageInfoDTOs
        } else {
    ...
    handleDelete(row) {
      this.$confirm('确定要报废此单据吗？', '提示', { type: 'warning'}).then(action => {
        if (action === 'confirm') {
          modifyFinishProductStorageScrap({ reciptNumber: row.recipt_number }).then(
            res => {
              if (res.status === 0) {
