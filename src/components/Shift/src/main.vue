<!-- 刘川中 2018.4.11 -->
<template>
  <el-dialog :title="title || '选择班次' " :visible.sync="visible" class="mes-dialog" width="40%">
    <el-col class="toolbar">
      <el-form :inline="true" :model="filters" @submit.native.prevent class="text-right">
        <el-form-item>
          <el-input v-model="filters.className" placeholder="输入关键字" @keyup.native.enter="getDataList" size="small"></el-input>
        </el-form-item>
...
<script>
  ...
  import { getShiftList } from '../../../api/api'
  export default {
    name: 'shift',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        getShiftList(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.classes
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.role_class_id === this.selectedIds[0]) {
              this.$nextTick(() => {
