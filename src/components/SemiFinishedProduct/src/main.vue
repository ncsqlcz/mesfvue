<!-- 刘川中 2018.6.8 -->
<template>
  <el-dialog :title="title || '选择半成品' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="semi_finish_name" label="半成品名称" sortable></el-table-column>
      <!-- <el-table-column prop="warehouse_material_batch" label="批次" sortable></el-table-column> -->
      <el-table-column prop="semi_finish_describe" label="半成品描述" width="200" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { querySemiFinishedProductModel } from '../../../api/api'
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        querySemiFinishedProductModel(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.SemiFinishs
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.semi_finish_id === this.selectedIds[0]) {
              this.$nextTick(() => {
