...
const SemiFinishedProduct = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, SemiFinishedProduct.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
SemiFinishedProduct.setDefaults = defaults => {
  SemiFinishedProduct.defaults = defaults
}

// 显示弹框
SemiFinishedProduct.show = (title, options) => {
  ...
  return SemiFinishedProduct(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
SemiFinishedProduct.close = () => {
  this.action = ''
  this.visible = false
}

export default SemiFinishedProduct
export { SemiFinishedProduct }
