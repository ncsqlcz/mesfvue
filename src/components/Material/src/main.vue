<!-- 刘川中 2018.6.8 -->
<template>
  <el-dialog :title="title || '选择物料' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column v-show="params.model===''" prop="material.warehouse_material_name" label="物料名称" sortable></el-table-column>
      <el-table-column v-show="params.model===''" prop="material.warehouse_material_standard" label="规格" sortable></el-table-column>
      <el-table-column v-show="params.model!==''" prop="warehouse_material_batch" label="批次" sortable></el-table-column>
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if ((item.warehouse_material_id ? item.warehouse_material_id : item.material.warehouse_material_id) === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if ((item.warehouse_material_id ? item.warehouse_material_id : item.material.warehouse_material_id) === this.selectedIds[0]) {
              this.$nextTick(() => {
