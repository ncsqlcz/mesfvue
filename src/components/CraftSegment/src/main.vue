<!-- 刘川中 2018.4.17 -->
<template>
  <el-dialog :title="'选择工艺段' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="craft_segment_basics_describe" label="工艺段备注" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { queryCraftSegmentBasics, queryCraftSegmentOutline } from '../../../api/api'
  export default {
    name: 'craft-segment',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        if (this.title) {
        queryCraftSegmentBasics(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.count
            this.list = data.map.craftSegmentDasicsList
          ...
          queryCraftSegmentOutline(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.count
            this.list = data.map.craftSegment
            this.checkSelectedIds()
          } else {
            this.filters.total = 0
            this.list = []
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.craft_basics_id === this.selectedIds[0]) {
              this.$nextTick(() => {
