...
const CraftSegment = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, CraftSegment.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
CraftSegment.setDefaults = defaults => {
  CraftSegment.defaults = defaults
}

// 显示弹框
CraftSegment.show = (title, options) => {
  ...
  return CraftSegment(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
