 ...
 * @Last Modified time: 2018-09-12 15:48:02
 */
<template>
  <el-dialog :title="title || '选择仓库' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="warehouse_number" label="仓库编号" sortable></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.warehouse_id).forEach(id => {
          if (id === row.warehouse_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.warehouse_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.warehouse_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { warehouse_id: id }
          })
        } else {
          // 选择单行
      ...
      getDataList () {
        ...
        queryWarehouse(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.line
            this.list = data.map.wareHouseInfoDTOs
