...
const WorkStepBasic = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, WorkStepBasic.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
WorkStepBasic.setDefaults = defaults => {
  WorkStepBasic.defaults = defaults
}

// 显示弹框
WorkStepBasic.show = (title, options) => {
  ...
  return WorkStepBasic(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
