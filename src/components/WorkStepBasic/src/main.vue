<!-- 刘川中 2018.4.17 -->
<template>
  <el-dialog :title="title || '选择工步名称' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="craft_workstep_basics_describle" label="备注" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { queryStepBasics } from '../../../api/api'
  export default {
    name: 'work-step-basic',
    data () {
      return {
        filters: {
          keyword: '',
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryStepBasics(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.counts
            this.list = data.map.resultList
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.craft_workstep_basics_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.craft_workstep_basics_id === this.selectedIds[0]) {
              this.$nextTick(() => {
