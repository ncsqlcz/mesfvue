...
<template>
  <el-dialog :title="title || '选择供应商' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="supplier_property" label="性质"></el-table-column>
      <el-table-column prop="supplier_address" label="地址"></el-table-column>
      <el-table-column prop="supplier_phone" label="电话" width="200"></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        getSupplierList(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.suppliers
