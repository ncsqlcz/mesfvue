<!-- 刘川中 2018.4.8 -->
<template>
  <el-dialog :title="title || '选择职位' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="role_post_detail" label="说明" width="200" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
    ...
    methods: {
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.role_post_id === this.selectedIds[0]) {
              this.$nextTick(() => {
