...
const DeviceType = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, DeviceType.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
DeviceType.setDefaults = defaults => {
  DeviceType.defaults = defaults
}

// 显示弹框
DeviceType.show = (title, options) => {
  ...
  return DeviceType(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
