...
<template>
  <el-dialog :title="title || '选择设备类型' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="devices_control_devices_type_name" label="设备类型" sortable></el-table-column>
...
<script>
  ...
  import { queryDeviceTypes } from '../../../api/api'
  export default {
    name: 'device-type',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryDeviceTypes(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines || 0
            this.list = data.map.devices_control_devices_type
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.devices_control_devices_type_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.devices_control_devices_type_id === this.selectedIds[0]) {
              this.$nextTick(() => {
