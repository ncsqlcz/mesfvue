...
const MPlanBatch = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, MPlanBatch.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
MPlanBatch.setDefaults = defaults => {
  MPlanBatch.defaults = defaults
}

// 显示弹框
MPlanBatch.show = (title, options) => {
  ...
  return MPlanBatch(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
