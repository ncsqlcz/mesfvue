...
const ProductType = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, ProductType.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
ProductType.setDefaults = defaults => {
  ProductType.defaults = defaults
}

// 显示弹框
ProductType.show = (title, options) => {
  ...
  return ProductType(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
