<!-- 刘川中 2018.3.28 -->
<template>
  <el-dialog :title="title || '产品类型' " :visible.sync="visible" class="mes-dialog" width="45%">
    ...
    >
      ...
      <el-table-column prop="product_type_name" label="类型名称" sortable></el-table-column>
      <el-table-column prop="product_type_number" label="编号" sortable></el-table-column>
      <el-table-column prop="product_type_describe" label="描述" width="200" sortable></el-table-column>
    </el-table>
    <!--产品类型列表 end-->
...
<script>
  ...
  import {getProductTypeList} from '../../../api/api'
  export default {
    name: 'product-type',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        getProductTypeList(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.productTypes
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.product_model_type_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.product_model_type_id === this.selectedIds[0]) {
              this.$nextTick(() => {
