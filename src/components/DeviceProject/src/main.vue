<!-- 刘川中 2018.4.3 -->
<template>
  <el-dialog :title="title || '选择设备项目' " :visible.sync="visible" class="mes-dialog" width="40%">
    <el-col class="toolbar">
      <el-form :inline="true" :model="filters" @submit.native.prevent class="text-right">
        <el-form-item>
          <el-input v-model="filters.projectName" placeholder="输入关键字" @keyup.native.enter="getDataList" size="small"></el-input>
        </el-form-item>
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="devices_upkeep_project_period" label="保养周期" sortable></el-table-column>
      <el-table-column prop="devices_upkeep_project_rank" label="保养级别" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryUpkeepPlans(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.upkeepProjects
