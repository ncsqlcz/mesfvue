...
const DeviceProject = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, DeviceProject.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
DeviceProject.setDefaults = defaults => {
  DeviceProject.defaults = defaults
}

// 显示弹框
DeviceProject.show = (title, options) => {
  ...
  return DeviceProject(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
