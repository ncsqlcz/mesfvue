<!-- 刘川中 2018.4.3 -->
<template>
  <el-dialog :title="title || '选择工单' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="plan.production_plan_batch_number" label="工单生产批号" sortable></el-table-column>
      <el-table-column prop="work_order_number" label="工单号" sortable></el-table-column>
      <el-table-column prop="work_order_responsible" label="工单负责人" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryWorkOrderOutline(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.line
            this.list = data.map.workOrders
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.work_order_id === this.selectedIds[0]) {
              this.$nextTick(() => {
