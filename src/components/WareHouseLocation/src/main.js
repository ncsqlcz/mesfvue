...
const WareHouseLocation = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, WareHouseLocation.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
WareHouseLocation.setDefaults = defaults => {
  WareHouseLocation.defaults = defaults
}

// 显示弹框
WareHouseLocation.show = (title, options) => {
  ...
  return WareHouseLocation(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
WareHouseLocation.close = () => {
  this.action = ''
  this.visible = false
}
