...
const ProductModel = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, ProductModel.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
ProductModel.setDefaults = defaults => {
  ProductModel.defaults = defaults
}

// 显示弹框
ProductModel.show = (title, options) => {
  ...
  return ProductModel(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
