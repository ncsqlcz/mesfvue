<!-- 刘川中 2018.4.17 -->
<template>
  <el-dialog :title="title || '选择工步' " :visible.sync="visible" class="mes-dialog" width="65%">
    <el-col class="toolbar">
      <el-form :inline="true" :model="filters" @submit.native.prevent class="text-right">
        <el-form-item>
          <el-select v-model="filters.productTypeId" placeholder="选择所属产品类型" size="small" @change="getDataList">
            <el-option
        ...
        <el-form-item>
          <el-select v-model="filters.polarity" placeholder="选择极性" size="small" @change="getDataList">
            <el-option
              v-for="item in polarityList"
              :key="item.value"
              :label="item.label"
              :value="item.value">
            </el-option>
        ...
        <el-form-item>
          ...
          <el-button type="" @click.native="reset" size="small">重置</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    <!--列表 start-->
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="product_polar" label="极性" sortable></el-table-column>
      <el-table-column prop="craft_control_workstep_version" label="工步版本" sortable></el-table-column>
      <el-table-column prop="product_type_name" label="所属产品类型" sortable></el-table-column>
      <el-table-column prop="product_model_genre" label="所属产品型号" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { queryStepOutline, getProductTypeList } from '../../../api/api'
  export default {
    name: 'work-step-outline',
    data () {
      return {
        filters: {
          productTypeId: '',
    ...
    methods: {
          ...
          if (data.status === 0 || data.map.productTypes.length > 0) {
            data.map.productTypes.map(v => {
              this.productTypeList.push({
                label: v.product_type_name,
                value: v.product_model_type_id
              })
            })
          }
        })
      ...
      getDataList () {
        ...
        queryStepOutline(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.counts
            this.list = data.map.workstepList
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.craft_control_workstep_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.craft_control_workstep_id === this.selectedIds[0]) {
              this.$nextTick(() => {
