<!-- 刘川中 2018.4.17 -->
<template>
  <el-dialog :title="title || '选择工艺' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="craft_basics_describe" label="备注" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { queryCraftBasics } from '../../../api/api'
  export default {
    name: 'craft',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryCraftBasics(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.count
            this.list = data.map.craftBasicsList
