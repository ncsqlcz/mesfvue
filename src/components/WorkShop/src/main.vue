<!-- 刘川中 2018.4.3 -->
<template>
  <el-dialog :title="title || '选择生产车间' " :visible.sync="visible" class="mes-dialog" width="40%">
    <el-col class="toolbar">
      <el-form :inline="true" :model="filters" @submit.native.prevent class="text-right">
        <el-form-item>
          <el-input v-model="filters.workshopName" placeholder="输入关键字" @keyup.native.enter="getDataList" size="small"></el-input>
        </el-form-item>
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="role_workshop_principal" label="负责人" sortable></el-table-column>
      <el-table-column prop="role_workshop_describe" label="备注" width="200" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
...
<script>
  ...
  import { queryWorkShopInfos } from '../../../api/api'
  export default {
    name: 'workshop',
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        queryWorkShopInfos(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.lines
            this.list = data.map.workshopInfos
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.role_workshop_id === this.selectedIds[0]) {
              this.$nextTick(() => {
